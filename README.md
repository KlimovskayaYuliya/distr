...
Помимо того, что функция может быть попросту плохо написана, мы также рискуем переполнить стек(Стек — это такая структура данных, которая работает по принципу «Last In, First Out» (последним пришёл — первым ушёл). Таким образом, элемент «проталкивается» в стек и добавляется в его конец, а затем «выталкивается» из стека при удалении.), что в конечном итоге приведёт к снижению скорости и программным ошибкам.
## 3. Репликация
